  ...
  return (
    <div style={{ width: '600px', height: '500px', border: '2px solid #333' }}>
      <Canvas camera={{ position: [5, 5, 5] }}>
        ...
        {/* energy vector (red) */}
        <arrowHelper args={[energyVec.clone().normalize(), new THREE.Vector3(), energyVec.length(), 'red']} />
        <Text position={[energyVec.x + 0.5, 0, 0]} fontSize={0.3} color="red">
          Energy {hormones.energy.toFixed(2)}
        </Text>

        {/* curiosity (green) */}
        <arrowHelper args={[curiosityVec.clone().normalize(), new THREE.Vector3(), curiosityVec.length(), 'lime']} />
        <Text position={[0, curiosityVec.y + 0.5, 0]} fontSize={0.3} color="lime">
          Curiosity {hormones.curiosity.toFixed(2)}
        </Text>

        {/* stress (blue) */}
        <arrowHelper args={[stressVec.clone().normalize(), new THREE.Vector3(), stressVec.length(), 'blue']} />
